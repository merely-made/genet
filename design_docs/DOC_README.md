...
| [Realms](2026-09-08_realms_plan.md) | One `Runtime` per agent, one realm per browsing context, stable `WindowProxy`, child/top navigation and ordinary/shadow/template adoption are landed. Latest closure, 2026-09-13: `Location.ancestorOrigins` snapshots, policy masking and detached-list lifetime. Focused tests pass on both engines; 132 WPT file/engine cases gain 49 Boa subtest passes with zero losses, and no-browsing-context passes 46/46. Three matching headed captures per engine preserve digest, 31 -> 31 nodes and collection. Open: `window.origin`, live canvas adoption, parser timing and the named navigation surfaces. Nova WPT no-results and DOM guard throughput remain qualified; no baseline repins. |
