...
| [Realms](2026-09-08_realms_plan.md) | One `Runtime` per agent, one realm per browsing context, stable `WindowProxy`, child/top navigation and ordinary/shadow/template adoption are landed. Latest closure, 2026-09-13: wrapper identity through source disposal and another adoption, including directed template retention. 657 engine/runtime tests pass; 86 WPT file/engine cases retain their outcomes and all 2,112 Boa passes. Three matching headed captures per engine preserve digest and 31 -> 31 nodes, prove detached collection and exit normally. Earlier external-script, Location and host-teardown receipts remain in the plan. Open: live canvas adoption, `ancestorOrigins`, parser timing and the named navigation surfaces. Nova WPT no-results and DOM guard throughput remain qualified; no baseline repins. |
