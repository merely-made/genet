...
| [Realms](2026-09-08_realms_plan.md) | One `Runtime` per agent, one realm per browsing context, stable `WindowProxy`, child/top navigation and ordinary/shadow/template adoption are landed. Latest closure, 2026-09-13: `window.origin`, worker environment origin and relative worker URL resolution. All 55 focused tests pass; 16 WPT file/engine cases gain four Boa subtest passes with zero losses, closing both blocked ancestor-origin files. Three headed captures per engine preserve digest, 31 -> 31 nodes and collection. Open: blank/srcdoc document URL separation, live canvas adoption, parser timing and named navigation surfaces. Nova WPT no-results and DOM guard throughput remain qualified; no baseline repins. |
