...
| [Realms](2026-09-08_realms_plan.md) | One `Runtime` per agent, one realm per browsing context, stable `WindowProxy`, navigation and ordinary/shadow/template adoption are landed. Latest closure, 2026-09-13: blank/srcdoc document URLs separated from inherited fallback bases, with resource and child CSS consumers updated. All 89 focused tests pass; 14 WPT file/engine cases preserve 50 Boa passes and gain one. Three headed captures per engine preserve digest, 31 -> 31 nodes and collection. Open: broader base-element/lifecycle behavior, live canvas adoption, parser timing and named navigation surfaces. Nova WPT no-results and DOM guard throughput remain qualified; no baseline repins. |
