...
| [MallocSizeOf and allocator removal](2026-09-22_malloc_size_of_removal_plan.md) | Servo's `about:memory` trait, derived across paint-types, xpath and the media crates with no reporter ever calling it, and the inherited `#[global_allocator]` that installed jemalloc on non-Windows builds through it. Founded 2026-09-22 under Mark's ruling: full removal, the allocator dropped so the host chooses, breaking publishes of `genet-paint-types` 0.2.0, `engine-observables-api` 0.2.0 and `genet-scripted-dom` 0.1.2 authorized. Script-free Ortet's dependency cone measures 401 -> 353 crates; the plan carries the suite, native-receipt and publish gates. |
