...
| [cadency, the owned selector engine](2026-09-18_cadency_selector_engine_plan.md) | Removes what the servo/stylo repository still supplied after the fork left: `stylo_malloc_size_of`, `servo_arc`, and the `selectors` crate, replaced by an owned, spec-shaped standalone crate for the slice Livery enables. Landed in the working tree 2026-09-18: `components/cadency` (10 tests), Livery's `selector.rs` 514 to 179 lines, `cargo tree -i` finds none of the removed crates, all suites at their 2026-09-16 counts, genet-wpt geometry relabelled `buckram`, and WPT before/after over lane F's ten runs plus `shadow-dom` testharness with zero transitions. Open: publication (cadency claim, breaking bumps), which is Mark's; a preserved `imported_part` direction bug in genet-livery's adapter; `script-runtime-api`'s separate `querySelector` selector module as a second consumer. |
